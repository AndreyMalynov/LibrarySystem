    ...
    class CloudSource : IData
    {
        ...
        public void OpenOrCreateBooks()
        {
            try
            {
                ...
                //выбросит исключение если файл еще не создан BooksFromCloud
                try
                {
                    using (FileStream fs = new FileStream( ("../../AppData/BooksFromCloud.xml"), FileMode.OpenOrCreate))
                    {
        ...
        public void SaveBooksChanges()
        {
            try
            {
                using (FileStream fs = new FileStream(("../../AppData/BooksFromCloud.xml"), FileMode.Create))
                {
                    formatterBooks.Serialize(fs, Books);
                }
            }
